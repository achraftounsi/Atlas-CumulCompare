...
def timed_excedance(nowcast_linda, last_timestep):
    return {last_timestep+datetime.timedelta(minutes=10*(e+1)) : nowcast_linda[e,:,:] for e in range(nowcast_linda.shape[0])}


def timed_nowcast(nowcast_linda, last_timestep):
    return {last_timestep+datetime.timedelta(minutes=10*(e+1)) : nowcast_linda[:,e,:,:] for e in range(nowcast_linda.shape[1])}
